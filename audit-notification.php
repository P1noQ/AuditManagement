...
<script> 
		...
		$result4 = mysql_query("select users.id,users.name as Name,users.family as Family from userinrole inner join users on users.id=userinrole.userid inner join role on role.id=userinrole.roleid where role.id=1");

		echo'
			function CreateAuditplaneform(number) {
			document.getElementById("tabl").innerHTML ="<tr><td><center>No.</center></td><td><center>Audit Scope</center></td><td><center>Objective</center></td><td><center>Audit_Date</center></td><td><center>Audit_Location</center></td><td><center>Auditors</center></td><td><center>Reference</center></td><td><center>Designation</center></td></tr>";
			for(i=1;i<=number;i++)
			{';
				echo 'document.getElementById("tabl").innerHTML +=';echo '"<tr>';echo '<td>"+i+"</td>';$alef='"+i+"';echo '<td>'; echo "<select name='scops$alef' style='width:90px'>";while($rows = mysql_fetch_array($result2)){echo "<option value='$rows[id]'>";echo $rows['Scope_Name'];echo '</option>';};echo '</select>';echo '</td>';echo "<td><center><textarea rows='5' name='part$alef' cols='7'></textarea></center></td>";echo'<td>';echo"<center><input type='date' style='width:80px' name='Date$alef' /></center>";echo'</td>';echo "<td><center><input type='Text' name='Location$alef' style='width:100px'/></center></td>";echo '<td>'; echo "<center><select name='ScopeAud$alef' style='width:90px'>";while($rowas = mysql_fetch_array($result4)){echo "<option value='$rowas[id]'>";echo "Mr .".$rowas['Name'].$rowas['Family'];echo '</option>';};echo '</select></center>';echo '</td>';echo "<td><center><textarea rows='9' name='Reference$alef' cols='7'></textarea></center></td>";echo "<td><center><textarea rows='9' name='Designation$alef' cols='9'></textarea>				</center></td>";echo '</td>';echo '</tr>"';
        		echo'}
		}';
...
?>
    ...
    <div id="site_content">
      <div id="content">
		...
		Enter row number : <input type="text" id="SendNumber" name="SendNum"/>
		<button type="button" style="width:15%;" onclick="CreateAuditplaneform(SendNumber.value)">create row</botton>
			...
			<tr>
				...
				<td>
					<center>Audit_Date</center>
				</td>
				<td>
					<center>Audit_Location</center>
				</td>
				<td>
...
<input type="submit" value="Submit" name="juju">
      </div>
    </div>
